...
var jourChoisi = "2021-09-12"; // Chargement des donnees
...
d3.csv("covid-06-11-2021.csv").then(function (data) {
  ...
  d3.json("departements-version-simplifiee.geojson").then(function (json) {
    ...
    for (var j = 0; j < json.feature.length; json++) {
      var departement = json.features.properties.nom;
      var jourDepchooisi = cleanData.find(function (row) {
        return row.jour = jourChoisi;
      });
      json.features[j].properties.value = jourDepchooisi.hosp;
    }

    g.selectAll("path").data(json.features).join("path").attr("d", path).style("fill", function (d) {
...
//# sourceMappingURL=/D3js-main.e31bb0bc.js.map